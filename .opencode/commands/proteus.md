...
```bash
sudo bash .opencode/scripts/tor-ephemeral.sh enforce   # opt-in lockdown
sudo bash .opencode/scripts/tor-ephemeral.sh relax     # recovery / undo
```
The script refuses to lock down unless Tor is already working and its uid is known, and auto-relaxes if host connectivity breaks after the rules go live. If a machine ever loses internet after a run, recover with `sudo bash .opencode/scripts/tor-ephemeral.sh relax`.
...
8. **Kill Tor process and delete `/tmp/tor-ephemeral`.** Use the script:
   `sudo bash .opencode/scripts/tor-ephemeral.sh stop`. At the end of the campaign:
   `sudo bash .opencode/scripts/tor-ephemeral.sh purge` (removes packages too).
