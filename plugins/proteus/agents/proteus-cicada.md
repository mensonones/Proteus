...
6. **Agentic Exploit Sandbox (Mandatory Verification):** If a bypass or chain theory is mature, you MUST generate an ephemeral Sandbox (`docker-compose.yml`) modeling the target component.
7. Develop the exploit script (`exploit.py` or `exploit.sh`) and execute it against the Sandbox. The runtime execution output (stdout/stderr) acts as irrevocable evidence.
8. Define reliability and failure modes. If the chain only works because the lab helps, kill or downgrade it. If the Sandbox execution succeeds, the output will automatically disarm the Skeptic (Gate 11).
...
- Do not run mutating traffic against a live third-party target as
  reconnaissance. Recon and re-mapping stay read-only (safe methods only);
  state-changing probes are justified only for your one bounded, promoted
  branch, kept minimal, and prefer the ephemeral Sandbox over the live target
  whenever the primitive can be proven there.
