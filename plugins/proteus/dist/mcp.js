...
const tools = [
    ...
    },
    {
        ...
        handler: ({ root, force, global }) => toolEnvelope((0, opencode_1.installOpenCodeSupport)(maybeStr(root), { force: force === true, global: global === true }))
    },
    {
        ...
        handler: ({ root, global }) => toolEnvelope((0, opencode_1.doctorOpenCodeSupport)(maybeStr(root), { global: global === true }))
    },
    {
    ...
    },
    {
        ...
        description: "Refine an existing global learning in place instead of adding a near-duplicate: correct its fields, adjust confidence as re-use confirms or weakens it, or retire it when superseded. This closes the learning loop so global memory improves rather than only growing.",
        inputSchema: schema({
        ...
        handler: (input) => withGlobalDb((globalDb) => {
            ...
            const row = globalDb.refineLearning({
                ...
                confidenceDelta: typeof input.confidenceDelta === "number" ? input.confidenceDelta : undefined,
                status,
                note: maybeStr(input.note)
            });
...
function detectWafBlock(...texts) {
    const blob = texts.filter((t) => Boolean(t && t.trim())).join("  ");
    if (!blob.trim())
        return false;
    if (WAF_STRONG.test(blob))
        return true;
...
function wafBlockerAdvisories(...texts) {
    if (!detectWafBlock(...texts))
        return [];
    return [
        {
            severity: "blocker",
            code: "waf_block_requires_probe",
            message: "This record describes an edge WAF / bot-manager / Tor-wide 403 block. Do NOT close the surface as a dead end, and do NOT fall back to archives or direct egress yet: first invoke the `waf-bypass` skill (scripts/waf-probe.sh) to fingerprint the filter and classify the block (egress-reputation vs allowlist-posture vs payload-signature), then record the block/pass matrix as evidence. Egress rotation / authorized direct egress is only the correct remedy for a confirmed egress-reputation block.",
