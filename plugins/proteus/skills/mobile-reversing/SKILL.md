...
9. Build candidate findings only from reachable mobile behavior. Tie each claim
   ...
   or native function.
10. Route credible exploitability work to `poc-exploit`; route broader research
   expansion to `chaining` only after the mobile entry point is concrete.
...
- Network routing for dynamic analysis: route traffic through Tor by prefixing
  commands with `proxychains4` (do not export `ALL_PROXY`/`HTTP_PROXY`/
  `HTTPS_PROXY` — they conflict with proxychains and break connections) unless
  the mobile target requires direct interception via mitmproxy.
