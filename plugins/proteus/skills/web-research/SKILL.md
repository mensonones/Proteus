...
surface were dead: first invoke the `waf-bypass` skill (`scripts/waf-probe.sh`)
...
5. Hand narrow input-reaction questions to `fuzzing`; hand side-effect chains to
   ...
   to `web-intel`; hand edge-WAF / bot-manager / Tor-wide 403 blocks that hide an
   in-scope surface to `waf-bypass` before treating the surface as unreachable.
