...
function renderGates(gates: ReturnType<ProteusDb["listValidationGates"]>): string {
  const rows = gates
    .map(
      (gate) =>
        `| G${gate.id} | ${gate.entityType}#${gate.entityId} | ${gate.gate} | ${gate.status} | ${gate.evidenceIds.join(", ") || "-"} | ${gate.summary || "-"} |`
    )
    .join("\n");
